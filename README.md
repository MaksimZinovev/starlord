...
5. Get a fit check matrix, ranking scores, and a ranked recommendation. Every claim links to a source.
6. A guard re-checks every rating against the rule text and writes a verdict to `validator-report.json`. Close calls, a gap within 1.0 points between #1 and #2, defer to you.
7. A script validates that every claim traces to a real fact, recomputes every ranking score, and checks the guard verdict.
...
├── validator-report.json conformance guard verdict
